...
public final class EntityCounter {
  ...
  private init(context: ModelContext?, for models: [PersistentModelLimit], default limit: Int?) {
    ...
    let modelNames = models.map { Self.cacheName(for: $0.type) }.sorted().joined(separator: "_")
    ...
    for (modelType, modelLimit) in models {
      let name = Self.cacheName(for: modelType)
    ...
    observationTask = Self.makeObservationTask(for: self)
  }
}
...
extension EntityCounter {
  ...
  /// Creates a task that performs the initial live fetch and observes context saves.
  private static func makeObservationTask(for counter: EntityCounter) -> Task<Void, Never> {
    Task { @MainActor [weak counter] in
      guard let context = counter?.context else {
        counter?.logger.error("Tracking started with nil context — counts will remain cached")
        counter?.isLoaded = true
        return
      }

      counter?.logger.info(
        "EntityCounter starting tracking for \(counter?.models.count ?? 0) models")

      // Populate live counts before entering the save-observation loop.
      counter?.refresh()
      counter?.isLoaded = true
      counter?.logger.info("EntityCounter initial refresh complete. isLoaded = true")

      for await note in NotificationCenter.default.notifications(named: ModelContext.didSave) {
        if Task.isCancelled { break }
        guard let counter else { break }
        guard let obj = note.object as? ModelContext, obj === context else { continue }
        counter.logger.debug("ModelContext.didSave detected — refreshing entity counts")
        counter.refresh()
      }
    }
  }
...
extension EntityCounter {
  ...
  public enum LimitScope: Sendable {
  ...
  public func updateLimit<T: PersistentModel>(_ newLimit: Int?, for modelType: T.Type) {
    ...
    totals[key] = current
    logger.info(
      "\(modelType, privacy: .public) limit updated to \(newLimit?.description ?? "unlimited")")
    refresh()
  }
}
...
extension EntityCounter {
  ...
  private func refresh() {
    ...
    for (modelType, configLimit) in models {
      do {
        ...
        if let oldEntry {
          effectiveLimit = oldEntry.freeLimit
        } else {
          effectiveLimit = configLimit
        }

        logChanges(
          for: modelType, oldCount: oldEntry?.count, newCount: newCount, freeLimit: effectiveLimit)
        totals[key] = Count(count: newCount, freeLimit: effectiveLimit)

        countSnapshot[Self.cacheName(for: modelType)] = newCount
      } catch {
        logger.error(
          "Failed to fetch \(String(describing: modelType)) count: \(error.localizedDescription, privacy: .public)"
        )
      }
    }
  ...
  private func logChanges(
    ...
    if let freeLimit {
      ...
      if !oldOver, newOver {
        logger.warning(
          "\(String(describing: modelType)) exceeded limit \(freeLimit, privacy: .public). Current count: \(newCount, privacy: .public)"
        )
      } else if oldOver, !newOver {
        logger.info(
          "\(String(describing: modelType)) is back under limit \(freeLimit, privacy: .public). Current count: \(newCount, privacy: .public)"
        )
      }
    }
  }
  ...
  /// Returns a module-qualified model name for cache isolation.
  private static func cacheName(for modelType: any FetchablePersistentModel.Type) -> String {
